...
public class SummonBuilderProcedure {
    ...
    public static void onWorldTick(LevelTickEvent.Post event) {
        SummonBuilderProcedure.execute((Event)event, (LevelAccessor)event.getLevel());
    }

    public static void execute(LevelAccessor world) {
        SummonBuilderProcedure.execute(null, world);
    }

    private static void execute(@Nullable Event event, LevelAccessor world) {
        if (InsideTheSystemModVariables.MapVariables.get((LevelAccessor)world).summon) {
            if (InsideTheSystemModVariables.MapVariables.get((LevelAccessor)world).isBlockEmpty) {
                if (world instanceof ServerLevel) {
                    ServerLevel _level = (ServerLevel)world;
                    Entity entityToSpawn = ((EntityType)InsideTheSystemModEntities.ANGRY_BUILDER.get()).spawn(_level, BlockPos.containing((double)(InsideTheSystemModVariables.MapVariables.get((LevelAccessor)world).PlayerX + 3.0), (double)InsideTheSystemModVariables.MapVariables.get((LevelAccessor)world).PlayerY, (double)InsideTheSystemModVariables.MapVariables.get((LevelAccessor)world).PlayerZ), MobSpawnType.MOB_SUMMONED);
                    if (entityToSpawn != null) {
                        entityToSpawn.setYRot(world.getRandom().nextFloat() * 360.0f);
                    }
                }
                InsideTheSystemModVariables.MapVariables.get((LevelAccessor)world).builderLook = true;
